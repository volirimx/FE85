...
// Task 3
const sortFilms = (arr) => arr.sort((a, b) => b.imdbRating - a.imdbRating);
console.log(sortFilms(films));
// Task 4
const newArrayFilms = (arr) => arr.map((item) => {
    return {
        id: item.id,
        title: item.title,
...
// Task 8
function sortFilmsByField(arr, field, value) {
    return arr.filter((item) => Object.keys(item).includes(field) && Object.values(item).includes(value));
}
