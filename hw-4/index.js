...
const getArrOfGenre = arr => Array.from(new Set(arr.reduce((acc, item) => acc.concat(item.genre), [])));
...
const createNewArr = arr => arr.map(item => {
  return {
    id: item.id,
    title: item.title,
...
// выхода фильма.
const filterArrByYear = (arr, year) => arr.filter(item => item.year == year);
...
// название фильма.
const filterArrByTitle = (arr, title) => arr.filter(item => item.title.toLowerCase().includes(title.toLowerCase()));
...
const filterArrByName = (arr, name) => arr.filter(item => item.title.toLowerCase().includes(name.toLowerCase())
|| item.plot.toLowerCase().includes(name.toLowerCase()));
...
// фильм с id = 2
const filterByKeyAndValue = (arr, key, value) => arr.filter(item => item[key] == value);
