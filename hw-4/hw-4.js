...
const getArrayOfFilms = (arr) => {
    let result = arr.map(item => item.genre).reduce((prev, item) => prev.concat(item));

    return [...new Set(result)]
}

// 2. Собрать в массив всех актеров всех фильмов (без повторения)
const getArrayOfActors = (arr) => {
    let result = arr.map(item => item.actors).reduce((prev, item) => prev.concat(item));
...
const getNewObject = (arr) => arr.map(item => ({id: item.id, title: item.title, released: item.released, plot: item.plot}));
 ...
 * выхода фильма.
 */
const filterFilmsByYear = (arr, number) => arr.filter(item => item.year === number);
 ...
 * название фильма.
 */
const filterFilmsByName = (arr, word) => arr.filter(item => item.title.toLowerCase().indexOf(word.toLowerCase()) >= 0);
 ...
 */
const filterFilmsByWord = (arr, word) => arr.filter(item => item.title.toLowerCase().indexOf(word.toLowerCase()) >= 0 || item.plot.toLowerCase().indexOf(word.toLowerCase()) >= 0);
 ...
 * фильм с id=2
 */
const NeZadanieAKakayatoHren = (arr, field, name) => arr.filter(item => Object.keys(item).includes(field) && Object.values(item).includes(name));
