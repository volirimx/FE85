...
// C массивом фильмов (films) сделать следующий
...
function getGenres (arr) {
    return arr.map(item => item.genre).join().split(',').filter((item, index, arra) => arra.indexOf(item) === index)
}

// 2

function getActors (arr) {
    return arr.map(item => item.actors).join().split(',').filter((item, index, arr) => arr.indexOf(item) === index)
}
...
function newFilms (arr) {
    return arr.map(item => ({id: item.id, title: item.title, released: item.released, plot: item.plot}))
}

// 5

function filterByReleaseYear (arr, date) {
...
function filterByTitleOrPlot (arr, str) {
    return arr.filter(item => item.title.toLowerCase().includes(str.toLowerCase()) || item.plot.toLowerCase().includes(str.toLowerCase()) )
}

// 8

function filterByParameter (arr, parameter, value) {
    return arr.filter(item => {
        if (typeof item[parameter] === 'string') {
            return item[parameter].toLowerCase().includes(value.toLowerCase())
        }
        else if (typeof item[parameter] === 'number' && item[parameter] === value) {
            return true
        }
    })
}
