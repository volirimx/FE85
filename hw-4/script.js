...
const getAllGenres = (arr) => {
   ...
   return arr.map(item => item.genre).join().split(',')
...
const getAllActors = (arr) => {

   return arr.map(item => item.actors).join().split(',')
      .filter((item, index, array) => array.indexOf(item) === index);
}
...
const sortFilmsByRating = (arr) => arr.sort((a, b) => b.imdbRating - a.imdbRating);

console.log(sortFilmsByRating(films));

//4. Создать новый массив, где объекты фильмов будут состоять из следующих полей:
//id, title, released, plot
const getNewArr = (arr) => arr.map((item) => ({ id: item.id, title: item.title, released: item.released, plot: item.plot }));

console.log(getNewArr(films));
...
//выхода фильма.
const filterFilmsByYear = (arr, date) => arr.filter(item => item.year === date);

console.log(filterFilmsByYear(films, 1977));
...
//название фильма.
const filterFilmsByTitle = (arr, title) => arr.filter(item => item.title.toLowerCase().includes(title.toLowerCase()));

console.log(filterFilmsByTitle(films, 'Black Widow'));
...
const filterFilmsByTitleOrPlot = (arr, str) => arr.filter(item => item.title.toLowerCase().includes(str.toLowerCase())
   || item.plot.toLowerCase().includes(str.toLowerCase()));

console.log(filterFilmsByTitleOrPlot(films, 'Harry Potter'));
...
//фильм с id=2
const filterFilmInfo = (arr, str, value) => arr.filter(item => Object.keys(item).includes(str) &&
   Object.values(item).includes(value));

console.log(filterFilmInfo(films, 'year', 1977));
