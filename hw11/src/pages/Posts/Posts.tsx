...
const Posts = ({ posts, loading, handleOnPageClick, postsCount }: IPostsProps) => {
    ...
    const getPosts = (): any =>  posts.map((post: IPost) => <PostMD id={post.id} key={post.id} image={post.image} text={post.description} date={post.createdAt} title={post.title } likesCount={post.likesCount} dislikesCount={post.dislikesCount}/>);
